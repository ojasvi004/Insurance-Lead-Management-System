	...
	public static void generateNewQuotation(Scanner sc) {
		...
		for (DataObjects.FNAQuestionResponseDO fna : fnaList) {
			...
			if (answer.equals("YES")) {
				...
				} else if (question.contains("retirement") || question.contains("lumpsum")
						|| question.contains("specific goal")) {
		...
		int policyChoice = 0;
		while (true) {
			policyChoice = InputValidation.getValidInt(sc,
					"Enter the number of the product you want to generate a quotation for: ");
		...
		String frequency = InputValidation.getValidOption(sc, "Enter payment frequency: ", freqOptions);
		...
		if (selectedPolicy.equalsIgnoreCase("Investment")) {
			...
			invPolicy.setPaymentFrequency(frequency);
